...
import { DeleteFamily } from "../../../services/familyServices/familyServices";
...
const MuiTableRow = ({
  ...
  const isItemSelected = isSelected(Family._id);
  ...
  const {
    mutateAsync: deleteFamily,
    isLoading: isMutatingDeleteFamily,
    isSuccess: isSuccessDeleteFamily,
  } = useMutation(DeleteFamily);
  ...
  const handleDeleteFamily = (idFamily: string) => {
    console.log("#idFamily", idFamily);

    deleteFamily(idFamily);
  };

  const queryClient = useQueryClient();

  useEffect(() => {
    if (isSuccessDeleteFamily) {
      setOpen(false);

      enqueueSnackbar("Famille supprimée avec succès", {
        variant: "success",
        anchorOrigin: {
          vertical: "bottom",
          horizontal: "center",
      ...
      queryClient.refetchQueries("families");

      setTimeout(() => {
        closeSnackbar();
      }, 5000);
    }
  }, [isSuccessDeleteFamily]);

  console.log("#Family ", Family);
  return (
    <TableRow
      hover
      // onClick={(event) => handleClick(event, Family.Rol_Code_V)}
      role="checkbox"
    ...
    >
      ...
      >
        <DialogTitle className={DialogClasses.alert_dialog_title}>
          <Trash />
          <span className="alert_dialog_title_text">Supprimer famille ?</span>
        </DialogTitle>
        <DialogContent className={DialogClasses.alert_dialog_content}>
          <DialogContentText>
            êtes vous sur de supprimer cette famille ?
          </DialogContentText>
        </DialogContent>
...
export default function RecepiesListEnhancedTable({
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = familiesData.map((n: any) => n._id);
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, familiesData?.length - page * rowsPerPage);
